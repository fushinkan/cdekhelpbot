...
async def process_role(message: Message, state: FSMContext):
    phone = await normalize_phone(message.text.strip())
    await StateUtils.prepare_next_state(message, state)
    ...
    async with async_session_factory() as session:
        ...
        try:
            ...
            if admin.role == "admin":
                ...
                await first_admin_login(message, state)
                return
        ...
        try:
            ...
            if user and user[0].role == "user":
                ...
                await first_client_login(message, state)
                return
